...
print("Emoji — T008 corrections:")
check(Emoji.forName("Marmalade") == "🍊", "marmalade → 🍊")
check(Emoji.forName("Lemon curd") == "🍋", "lemon curd → 🍋")
check(Emoji.forName("Chicken nugget") == "🍗", "chicken nugget → 🍗")
check(Emoji.forName("Ginger") == "🫚", "ginger → 🫚")
check(Emoji.forName("Galangal") == "🫚", "galangal → 🫚")
check(Emoji.forName("Turmeric") == "🫚", "turmeric → 🫚")
check(Emoji.forName("Assam") == "🍵", "assam → 🍵")
check(Emoji.forName("Periwinkle") == "🦪", "periwinkle → 🦪")
check(Emoji.forName("Kebab") == "🥙", "kebab → 🥙")
check(Emoji.forName("Comb") == "🪮", "comb → 🪮")
check(Emoji.forName("Hair brush") == "🪮", "hair brush → 🪮")
check(Emoji.forName("Nail") == "💅", "nail → 💅")
check(Emoji.forName("Birthday card") == "💌", "birthday card → 💌")
check(Emoji.forName("Squash") == "🎃", "squash → 🎃")
check(Emoji.forName("Butternut") == "🎃", "butternut → 🎃")
check(Emoji.forName("Ackee") == "🥭", "ackee → 🥭")
check(Emoji.forName("Mirin") == "🍶", "mirin → 🍶")
...
print("Emoji — T009 short-keyword exact-word matching:")
check(Emoji.forName("Toilet paper") != "🥣", "toilet paper no longer false-matches 'pap' → 🥣")
check(Emoji.forName("Paper towels") != "🥣", "paper towels no longer false-matches 'pap' → 🥣")
check(Emoji.forName("Carpet cleaner") != "🐠", "carpet cleaner no longer false-matches 'carp' → 🐠")
check(Emoji.forName("Hamper") != "🍖", "hamper no longer false-matches 'ham' → 🍖")
check(Emoji.forName("Pap") == "🥣", "pap alone still → 🥣")
check(Emoji.forName("Ham") == "🍖", "ham alone still → 🍖")
check(Emoji.forName("Hams") == "🍖", "hams (plural) still → 🍖")
check(Emoji.forName("Carp") == "🐠", "carp alone still → 🐠")
check(Emoji.forName("Figs") == "🍇", "figs (plural) still → 🍇")
check(Emoji.forName("Peas") == "🫛", "peas (plural) still → 🫛")
...
check(Measure.step(501, unit: .milliliter, up: true) == 550, "off-grid 501 ml +→ snaps up to 550, not 551")
check(Measure.step(501, unit: .milliliter, up: false) == 500, "off-grid 501 ml −→ snaps down to 500")
check(Measure.step(100, unit: .gram, up: true) == 150, "typed 100 g +→ 150 (steps from the typed value)")
check(Measure.step(100, unit: .gram, up: false) == 50, "typed 100 g −→ 50")
check(Measure.step(550, unit: .milliliter, up: false) == 500, "on-grid 550 ml −→ 500")
check(Measure.step(1.1, unit: .kilogram, up: true) == 1.25, "off-grid 1.1 kg +→ 1.25")
check(Measure.step(1.1, unit: .kilogram, up: false) == 1.0, "off-grid 1.1 kg −→ 1.0")
